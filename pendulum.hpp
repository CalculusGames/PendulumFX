...
class Pendulum {
  private:
    ...
    static void erasePendulum(
        int pivotX, int pivotY, int ballX, int ballY, bool drawTrail
    ) {
    ...
    static void drawPendulum(
        int pivotX, int pivotY, int ballX, int ballY, int color
    ) {
  ...
  public:
    Pendulum(
        double length, double mass, double angle, int x, int y,
        int color = 0xFFFFFF, bool draw = false
    )
        : length(length), mass(mass), angle(angle), x(x), y(y),
          angularVelocity(0.0), angularAcceleration(0.0), parent(nullptr),
    ...
    static void setGlobalDamping(double d) {
        globalDamping = d;
    }
    static void setGlobalCoupling(double c) {
        globalCoupling = c;
    }

    static void stepMultiPendulum(
        std::vector<Pendulum*> pendulums, double gravity, double timeStep
    ) {
        ...
        for (size_t i = 0; i < n; i++) {
            erasePendulum(
                pendulums[i]->prevPivotX, pendulums[i]->prevPivotY,
                pendulums[i]->prevBallX, pendulums[i]->prevBallY,
                pendulums[i]->draw
            );
        }
        ...
        if (n == 2) {
            ...
            double couplingTerm =
                -2 * sin(delta) * m2 *
                (bdot * bdot * l2 + adot * adot * l1 * cos(delta));
            ...
            double numerator2 =
                2 * sin(delta) *
                (angularMomentumTerm + gravityTermB + centrifugalTerm);
            ...
            // for n >= 3, use Lagrangian-based multi-body dynamics with
            // constraint forces
            std::vector<double> accelerations(n);

            // compute constraint forces and accelerations using iterative
            // method
            for (size_t i = 0; i < n; i++) {
                ...
                // constraint acceleration from masses below (Lagrangian
                // constraint force)
                if (totalMassBelow > 0.0) {
                    ...
                    for (size_t j = i + 1; j < n; j++) {
                        ...
                        double constraintTerm =
                            massWeight * (lj / li) / (pathLength * pathLength);

                        // centripetal acceleration constraint
                        constraintAccel +=
                            constraintTerm * omegaj * omegaj * sin(deltaTheta);

                        // angular coupling constraint with damping
                        constraintAccel += globalCoupling * constraintTerm *
                                           sin(deltaTheta) * 0.5;
                    }
                ...
                // coupling with adjacent pendulums (nearest neighbor
                // interaction)
                if (i > 0) {
                    ...
                    double parentCouplingStrength =
                        (mPrev / (mi + mPrev)) * (lPrev / li);
                    accel += parentCouplingStrength * omegaPrev * omegaPrev *
                             sin(deltaFromParent) * 0.3;
                }
                ...
                if (i + 1 < n) {
                    ...
                    double childCouplingStrength =
                        (mNext / (mi + mNext)) * (lNext / li);
                    accel += childCouplingStrength * omegaNext * omegaNext *
                             sin(deltaToChild) * 0.15;
                }
        ...
        for (size_t i = 0; i < n; i++) {
            pendulums[i]->angularVelocity +=
                pendulums[i]->angularAcceleration * timeStep;
        ...
        for (size_t i = 0; i < n; i++) {
            int ballX = currentPivotX +
                        static_cast<int>(
                            pendulums[i]->length * sin(pendulums[i]->angle)
                        );
            int ballY = currentPivotY +
                        static_cast<int>(
                            pendulums[i]->length * cos(pendulums[i]->angle)
                        );

            drawPendulum(
                currentPivotX, currentPivotY, ballX, ballY, pendulums[i]->color
            );
